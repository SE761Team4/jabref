    ...
    return (
        <Stage width={width} height={height} className={classes.canvas}>
            <Layer>
                ...
                {nodes.map((node) =>
                    <Node id={node.id} x={node.x} y={node.y} colors={node.colors} updateEdges={updateEdges} selectedNodeId={selectedNodeId} setSelectedNodeId={setSelectedNodeId} updateNode={updateNode}/>
                )}
            </Layer>
        </Stage>
    )
