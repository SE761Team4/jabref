...
const Node = ({node, updateEdges, setSelectedNode, selectedNodeId, updateNode, isInSearch}) => {
    ...
    const toggleFavouritedIcon = () => {
        if (node.icons) {
            ...
            if (node.icons.includes(IconTypes.FAVOURITE)) {
                newIcons = node.icons.filter((icon) => {
                    return icon !== IconTypes.FAVOURITE
                });
                newIcons.push(IconTypes.NOT_FAVOURITE);
            } else {
                newIcons = node.icons.filter((icon) => {
                    return icon !== IconTypes.NOT_FAVOURITE
                });
    ...
    const toggleReadIcon = () => {
        if (node.icons) {
            ...
            if (node.icons.includes(IconTypes.READ)) {
                newIcons = node.icons.filter((icon) => {
                    return icon !== IconTypes.READ
                });
                newIcons.push(IconTypes.TO_READ);
            } else {
                newIcons = node.icons.filter((icon) => {
                    return icon !== IconTypes.TO_READ
                });
    ...
    const togglePriority = () => {
        if (node.icons) {
            ...
            if (node.icons.includes("HIGH_PRIORITY")) {
                newIcons = node.icons.filter((icon) => {
                    return icon !== "HIGH_PRIORITY"
                });
                setPriorityColor(priorityColors[0]);
            } else if (node.icons.includes("MEDIUM_PRIORITY")) {
                newIcons = node.icons.filter((icon) => {
                    return icon !== "MEDIUM_PRIORITY"
                });
    ...
    return (
        ...
        >
            ...
            >
                <Image
                    image={node.icons.includes(IconTypes.READ) ? readIcon : toReadIcon}
                    width={24}
                    height={24}
                    offsetX={-50}
                    onClick={toggleReadIcon}/>
                ...
                <Image
                    ...
                    height={24}
                    offsetX={-20}
                    fill={priorityColor !== 'none' ? priorityColor : selectedNodeId === node.id ? "#a2b8e5" : "white"}
                    onClick={togglePriority}/>}

                <Image
                    image={node.icons.includes(IconTypes.FAVOURITE) ? favouritedIcon : notFavouriteIcon}
                    width={24}
                    height={24}
                    offsetX={-80}
                    onClick={toggleFavouritedIcon}/>
            </Group>
        </Group>
    );
};
